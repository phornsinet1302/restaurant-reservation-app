require('dotenv').config();
const { supabaseAdmin } = require('./config/supabase');

async function setupStorageBucket() {
  try {
    console.log('🔧 Setting up Supabase Storage bucket...\n');

    // Create the bucket
    console.log('1️⃣  Creating "restaurant-media" bucket...');
    const { data: createData, error: createError } = await supabaseAdmin.storage.createBucket(
      'restaurant-media',
      {
        public: true, // Make bucket public so videos can be accessed
      }
    );

    if (createError) {
      // If bucket already exists, that's fine
      if (createError.message.includes('already exists')) {
        console.log('   ℹ️  Bucket already exists');
      } else {
        console.error('   ❌ Error creating bucket:', createError.message);
        return;
      }
    } else {
      console.log('   ✅ Bucket created successfully');
      console.log('   - Name: restaurant-media');
      console.log('   - Public: true');
    }

    // Test 2: List buckets
    console.log('\n2️⃣  Verifying bucket...');
    const { data: buckets, error: listError } = await supabaseAdmin.storage.listBuckets();

    if (listError) {
      console.error('   ❌ Error listing buckets:', listError);
      return;
    }

    const bucket = buckets.find((b) => b.name === 'restaurant-media');
    if (bucket) {
      console.log('   ✅ Bucket verified');
      console.log('   - ID:', bucket.id);
      console.log('   - Public:', bucket.public);
    } else {
      console.log('   ❌ Bucket still not found');
      return;
    }

    // Test 3: Test upload
    console.log('\n3️⃣  Testing file upload...');
    const testFile = Buffer.from('Test file - ' + new Date().toISOString());
    const { data: uploadData, error: uploadError } = await supabaseAdmin.storage
      .from('restaurant-media')
      .upload(`test/test_${Date.now()}.txt`, testFile, {
        contentType: 'text/plain',
        upsert: false,
      });

    if (uploadError) {
      console.error('   ❌ Upload failed:', uploadError);
      return;
    }

    console.log('   ✅ Test upload successful');
    console.log('   - Path:', uploadData.path);

    // Test 4: Get public URL
    const { data: publicData } = supabaseAdmin.storage
      .from('restaurant-media')
      .getPublicUrl(uploadData.path);

    console.log('\n4️⃣  Public URL for videos:');
    console.log('   ' + publicData.publicUrl.split('/test/')[0] + '/videos/{filename}');

    // Test 5: Cleanup
    console.log('\n5️⃣  Cleaning up test file...');
    await supabaseAdmin.storage
      .from('restaurant-media')
      .remove([uploadData.path]);
    console.log('   ✅ Test file deleted');

    console.log('\n✅ Supabase Storage is ready for video uploads!');
    console.log('\n📝 Summary:');
    console.log('   - Bucket: restaurant-media');
    console.log('   - Public: Yes');
    console.log('   - Max file size: 100MB');
    console.log('   - Ready for: Images, Videos, Media');

  } catch (error) {
    console.error('\n❌ Setup error:', error.message);
  }
}

setupStorageBucket();
